  ...
  if (!business) {
    return (
      <div className="flex flex-col items-center justify-center py-24">
        ...
        <h2 className="text-[16px] font-semibold mb-2" style={{ color: '#e7e5e4' }}>Set up your business</h2>
        <p className="text-[13px] mb-4 text-center max-w-xs" style={{ color: '#78716c' }}>
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-[16px] font-semibold" style={{ color: '#e7e5e4' }}>Good morning</h2>
          <p className="text-[13px]" style={{ color: '#78716c' }}>{business.name}</p>
        </div>
            ...
            <div className="h-52">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData}>
                  ...
                  <XAxis dataKey="date" tick={{ fontSize: 11, fill: '#57534e' }} tickLine={false} axisLine={false} interval="preserveStartEnd" />
                  <YAxis tick={{ fontSize: 11, fill: '#57534e' }} tickLine={false} axisLine={false} allowDecimals={false} domain={[0, 'auto']} width={24} />
                  <Tooltip
                    contentStyle={{
                      background: '#322e2b',
                  ...
                  />
                  <Area type="monotone" dataKey="conversations" stroke="#f59e0b" strokeWidth={2} fill="url(#colorConv)" name="Conversations" />
          ...
          <Card>
            <div className="space-y-1">
              {[
                ...
                { label: "Active Agents", value: agents.filter((a) => a.is_active).length, icon: <Bot className="w-3.5 h-3.5" />, color: '#fbbf24' },
              ].map((item) => (
                <div key={item.label} className="flex items-center gap-3 py-2.5" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                  ...
                  <div className="flex-1">
                    <div className="text-[11px]" style={{ color: '#78716c' }}>{item.label}</div>
                    <div className="text-[14px] font-semibold" style={{ color: '#e7e5e4' }}>{item.value}</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      <Card>
        ...
        {recentAppointments.length === 0 ? (
          <div className="text-center py-8 text-[13px]" style={{ color: '#57534e' }}>
            ...
            {recentAppointments.map((appt, i) => (
              <div key={appt.id} className="flex items-center gap-4 py-3"
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-[13px] font-medium truncate" style={{ color: '#e7e5e4' }}>{appt.customer_name}</div>
                  <div className="text-[11px] truncate" style={{ color: '#57534e' }}>
                    {appt.vehicle_year} {appt.vehicle_make} {appt.vehicle_model}
                  </div>
                </div>
                <div className="text-right flex-shrink-0">
                  <div className="text-[11px]" style={{ color: '#78716c' }}>{formatDateTime(appt.scheduled_at)}</div>
