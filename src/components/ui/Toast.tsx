...
const CONFIGS: Record<ToastType, { icon: React.ReactNode; barColor: string }> = {
  success: { icon: <CheckCircle2 className="w-4 h-4 flex-shrink-0" style={{ color: '#fbbf24' }} />, barColor: '#f59e0b' },
...
function ToastItem({ toast, onDismiss }: { toast: Toast; onDismiss: (id: string) => void }) {
  ...
  return (
    <div
      ...
      style={{ background: '#322e2b', border: '1px solid rgba(255,255,255,0.1)', boxShadow: '0 8px 32px rgba(0,0,0,0.5)' }}
    >
      <div className="absolute bottom-0 left-0 right-0 h-[2px]" style={{ background: cfg.barColor }} />
      {cfg.icon}
      <div className="flex-1 min-w-0">
        <p className="text-[13px] font-semibold leading-tight" style={{ color: '#e7e5e4' }}>{toast.title}</p>
        {toast.message && <p className="text-[11px] mt-0.5 leading-relaxed" style={{ color: '#78716c' }}>{toast.message}</p>}
      </div>
      <button
