  ...
  return (
    <>
      ...
      <aside className={cn(
        ...
        style={{ background: '#1c1917', borderRight: '1px solid rgba(255,255,255,0.07)' }}>
        ...
        <div className="flex items-center gap-2.5 h-14 px-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
          ...
          <div className="flex flex-col leading-none">
            <span className="text-[13px] font-bold tracking-tight text-white">SuperVoice<span className="text-amber-400">AI</span></span>
            <span className="text-[10px] text-amber-500/70 font-medium mt-0.5">Voice Platform</span>
          </div>
        </div>
          ...
          <div className="px-3 py-2.5" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
            <div className="flex items-center gap-2 px-2.5 py-2 rounded-lg" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[10px] truncate" style={{ color: '#57534e' }}>{business.city || 'Configure location'}</div>
              </div>
          ...
          {navGroups.map((group) => (
            <div key={group.label}>
              <div className="px-2 mb-1.5">
                <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: '#44403c' }}>
                  {group.label}
                </span>
              </div>
                  ...
                  return (
                    <Link key={item.href} href={item.href} onClick={onClose}
                      className={cn('relative flex items-center gap-2.5 px-2.5 py-2 rounded-lg transition-all duration-100 group',
                        active ? 'text-amber-300 font-semibold' : 'font-medium hover:text-slate-300'
                      )}
                      style={{ color: active ? undefined : '#78716c' }}>
                      {active && (
                        <motion.div
                      ...
                      <item.icon className={cn('w-[15px] h-[15px] flex-shrink-0 relative z-10 transition-colors',
                        active ? 'text-amber-400' : 'text-[#44403c] group-hover:text-slate-400'
                      )} />
                      <span className="text-[13px] relative z-10 flex-1">{item.label}</span>
                      {active && <div className="w-1.5 h-1.5 rounded-full bg-amber-500 flex-shrink-0 relative z-10" />}
                    </Link>
                  );
                })}
              </div>
